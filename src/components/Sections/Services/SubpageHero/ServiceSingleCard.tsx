...
export function ServiceSingleCard({ card: { title, subtitle, category, image, alt, path } }: Card) {
	return (
		<div className='flex flex-col gap-2 p-4 rounded-2xl bg-white'>
			<div className='overflow-hidden relative flex flex-col items-end w-full p-3 aspect-4.5/3 rounded-xl '>
				<Image src={image} alt={alt} fill className='object-cover' />
				<div className='px-3 py-2 z-2 rounded-full bg-white'>
					<span className='font-semibold text-black/60'>{category}</span>
				</div>
			</div>
